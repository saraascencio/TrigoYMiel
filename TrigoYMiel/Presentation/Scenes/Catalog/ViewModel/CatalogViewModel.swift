    ...
    // MARK: - State
        ...
        func promotionType(for product: Product) -> PromotionType {
            ...
            let validPromos = activePromotions.filter { promo in
                ...
                promo.startDate <= now &&
                now <= promo.endDate &&
                promo.applicableProductIds.contains { id in
                    let idStr = "\(id)"
                    return idStr.contains(product.id) || product.id.contains(idStr)
                }
            }
            
            guard let bestPromo = validPromos.max(by: { $0.discountPercentage < $1.discountPercentage }) else {
                return .none
            }
            
            // Si es promo mayorista
            if bestPromo.wholesaleOnly {
                return (currentUser?.tier == .wholesale) ? .wholesale : .none
            }
            
            // Promo estándar (para todos)
            return .standard
        }
        ...
        var productsByCategory: [(category: ProductCategory, products: [Product])] {
            categories.compactMap { category in
                ...
                let products = allProducts.filter { $0.categoryId == category.id && $0.isAvailable }
